...
send_task_scoket = context.socket(zmq.PUSH)
send_task_scoket.bind("tcp://*:5557")
...
def add_files():
    ...
    # RAID1: cut the file in half and store both halves 2x
    file_data_1 = file_data[: math.ceil(size / 2)]
    file_data_2 = file_data[math.ceil(size / 2) :]
    ...
    # wait for 4 responses from storage nodes
    for task_no in range(4):
        resp = response_socket.recv_string()
        print(f'Response {task_no}: {resp}')

    # now all chunks are stored...
