...
class FileHandler:
    ...
    def store_file(self, file_bytes):
        ...
        fragments = [file_bytes[i:i+fragment_size] for i in range(0, len(file_bytes), fragment_size)]
        ...
        for replica_idx in range(NO_REPLICAS):
            for frag_idx, fragment in enumerate(fragments):
                ...
                t = threading.Thread(target=self.__upload_fragment_to_node, args=(node, file_id, frag_idx, fragment))
                t.start()
                threads.append(t)
        for t in threads:
            t.join()
    ...
    def __setup(self):
        if NODE_SELECTION_STRATEGY == RANDOM_SELECTION:
            self.node_selector = RandomSelection(NO_NODES, NO_REPLICAS)
        elif NODE_SELECTION_STRATEGY == MIN_COPY_SETS_SELECTION:
            self.node_selector = MinCopySetsSelection(NO_NODES, NO_REPLICAS)
        elif NODE_SELECTION_STRATEGY == BUDDY_SELECTION:
            self.node_selector = BuddySelection(NO_NODES, NO_REPLICAS)
        else:
            raise Exception(f"Invalid node selection strategy: {NODE_SELECTION_STRATEGY}")
